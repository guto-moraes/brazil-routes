...
function CarouselTimeline() {
  ...
  useGSAP(() => {
    ...
    const timelineWrapper: HTMLElement | null = document.querySelector("#timeline");
  ...
  return (
    <>
      <section id="" className="h-svh w-full bg-[#f3e7d9] grid place-content-center">
        <h1 className="text-8xl text-[#9c8b79] font-black uppercase">Hero</h1>
      </section>

      <section id="timeline" className="h-svh w-full flex flex-col">
        <div
          id="timeline-container"
          className="grow bg-mate-duo-400 h-full w-full overflow-hidden"
          ...
          {panels &&
            panels.map((panel, index) => (
              <article
                ...
                key={index}
              >
                <h2 className="text-8xl text-white font-black">Slide {panel.year}</h2>
              </article>
            ))}
        </div>

        <div className="bg-bone-900 h-16 w-full flex justify-center items-center">
      ...
      <section className="h-svh w-full bg-bone-200 grid place-content-center">
        <h1 className="text-8xl text-bone-600 font-black uppercase">Hero</h1>
      </section>
    </>
  );
}
