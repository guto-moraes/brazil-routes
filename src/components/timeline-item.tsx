...
const Heading2 = ({ text, textColor }: { text: string; textColor: string }) => (
  <h2 className="text-[clamp(1.75rem,4.5vw,3rem)] font-cabinet font-bold leading-[1.125] dark:text-dark-contrast-100!" style={{ color: textColor }}>
    {text}
  </h2>
);

const Heading3 = ({ text, textColor }: { text: string; textColor: string }) => (
  <h3 className="text-[clamp(1.5rem,5vw,2rem)] font-cabinet font-bold leading-[1.125] dark:text-dark-contrast-100!" style={{ color: textColor }}>
    {text}
  </h3>
);
...
const Paragraph = ({ content }: { content: string }) => {
  return <div className="text-bone-800 dark:text-white text-balance hyphens-auto" dangerouslySetInnerHTML={sanitizedData(content)} />;
};
    ...
    timelines.map((timeline) => {
      ...
      return (
        <article
          ...
          className="timeline-item h-svh xl:h-[calc(100svh-64px)] w-full xl:w-svw flex flex-col dark:bg-dark-950!"
        ...
        >
          ...
          >
            {events.map((date, idx) => (
              <ColHeader className={idx === 1 ? "flex-4 dark:text-dark-200!" : "flex-2 dark:text-dark-200!"} text={date} key={idx} color={timeline.info.textColor} />
            ))}
          ...
          <div className="h-full xl:grow min-w-5xl w-full flex divide-x divide-dashed divide-bone-900/20 dark:divide-dark-700">
            ...
            <div className="w-svw md:w-120 xl:w-svw xl:flex-4 flex flex-col justify-end gap-y-8 px-8 pb-8">
              <hr className="h-px border-px border-dashed border-bone-800/20 dark:border-dark-700 bg-none" />
              <div className="w-full flex flex-col gap-y-4">
