...
const InitialPage = ({ onStart }: { onStart: () => void}) => {
  ...
  return (
    <div className="w-full md:w-2/3 max-w-full mx-auto flex flex-col gap-y-8 max-sm:px-4">
      <h2 className="text-[clamp(1.25rem,4vw,2rem)] text-tan-700 dark:text-dark-contrast-100 font-cabinet font-black text-center">
        Informaçãoes <span className="text-tan-500 dark:text-dark-contrast-50">sobre o questionário</span>
      </h2>
      <div className="max-sm:px-4 grid grid-cols-3 gap-3 sm:gap-8">
        <div className="bg-bone-200 dark:bg-dark-800 rounded-2xl md:h-48 flex flex-col gap-2 justify-between items-center p-3 md:p-8">
          <CircleQuestionMark className="text-bone-500 dark:text-dark-300 size-[clamp(1.85rem,5vw,2.5rem)]" />
          <h2 className="text-bone-600 dark:text-dark-500 text-[clamp(1.5rem,5vw,2.25rem)] font-bold leading-none tracking-tighter">{data.page.quiz.totalQuestions}</h2>
          <h2 className="text-bone-500 dark:text-dark-200 font-medium leading-none">Questões</h2>
        </div>
        <div className="bg-bone-300/50 dark:bg-dark-900 rounded-2xl md:h-48 flex flex-col justify-between items-center p-3 md:p-8">
          <Clock className="text-bone-500 dark:text-dark-300 size-[clamp(1.85rem,5vw,2.5rem)]" />
          <h2 className="text-bone-600 dark:text-dark-500 text-[clamp(1.5rem,5vw,2.25rem)] font-bold leading-none tracking-tighter">{data.page.quiz.countdown}:00</h2>
          <h2 className="text-bone-500 dark:text-dark-200 font-medium leading-none">Tempo</h2>
        </div>
        <div className="bg-bone-200 dark:bg-dark-800 rounded-2xl md:h-48 flex flex-col gap-2 justify-between items-center p-3 md:p-8">
          <BadgeCheck className="text-bone-500 dark:text-dark-300 size-[clamp(1.85rem,5vw,2.5rem)]" />
          <h2 className="text-bone-600 dark:text-dark-500 text-[clamp(1.5rem,5vw,2.25rem)] font-bold leading-none tracking-tighter">100%</h2>
